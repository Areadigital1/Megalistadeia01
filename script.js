...
let badges = []; // Array para armazenar os selos
let credits = []; // Array para armazenar os créditos
let appSettings = {}; // Armazenar configurações do aplicativo
...
// Tradução de perfis de créditos
const creditRoles = {
  'creator': 'Criador',
  'editor': 'Editor',
...
try {
const snapshot = await database.ref('appSettings').once('value');
if (snapshot.exists()) {
appSettings = snapshot.val() || {};


// Atualizar logo do aplicativo se existir
if (appSettings.logo) {
updateAppLogo(appSettings.logo);
}
}
return appSettings;
} catch (error) {
...
// Atualiza o logo do aplicativo nos diferentes locais
...
loginLogoText.classList.remove('hidden');
};
...
// Carrega os selos (badges) do banco de dados
async function loadBadges() {
try {
const snapshot = await database.ref('badges').once('value');
badges = [];

if (snapshot.exists()) {
snapshot.forEach(childSnapshot => {
const badge = childSnapshot.val();
badge.id = childSnapshot.key;
badges.push(badge);
});
}
...
// Carrega os créditos do banco de dados
async function loadCredits() {
try {
const snapshot = await database.ref('credits').orderByChild('timestamp').once('value');
credits = [];

if (snapshot.exists()) {
snapshot.forEach(childSnapshot => {
const credit = childSnapshot.val();
credit.id = childSnapshot.key;
credits.push(credit);
});

// Modificado: Ordenação prioritária por role (criador primeiro) e depois por nome
credits.sort((a, b) => {
// Priorizar "creator" (criador) primeiro
if (a.role === 'creator' && b.role !== 'creator') return -1;
if (a.role !== 'creator' && b.role === 'creator') return 1;


// Depois verificar outras roles
if (a.role === b.role) {
  // Se a role for igual, ordenar por nome
  return a.name.localeCompare(b.name);
}
  
// Para outras roles, ordem alfabética
return creditRoles[a.role].localeCompare(creditRoles[b.role]);
});
}
...
// Atualiza a exibição dos créditos na barra lateral
function updateCreditsUI() {
creditsList.innerHTML = '';

if (credits.length === 0) {
creditsList.innerHTML = '<p class="text-gray-500 dark:text-gray-400 text-center text-sm">Nenhum crédito disponível</p>';
return;
}

credits.forEach(credit => {
const verifiedBadge = credit.verified ? 
...
const roleText = creditRoles[credit.role] || credit.role;
const roleElement = `<span class="user-badge">${roleText}</span>`;

const creditElement = document.createElement('div');
creditElement.className = 'flex items-center py-2';
creditElement.innerHTML = `
    <div class="w-8 h-8 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center text-primary">
        <span>${getInitials(credit.name)}</span>
    </div>
    <div class="ml-3">
        <p class="text-sm font-medium flex items-center">
            ${credit.name}${verifiedBadge}
        </p>
...
// Inicialização - Carregar logo e outras configurações
loadAppSettings();
...
const badgeHtml = badge ? `<div class="animated-badge">${badge.name}</div>` : '';
...
${badgeHtml}
<button class="favorite-btn absolute top-10 right-2 w-9 h-9 rounded-full bg-white dark:bg-gray-800 shadow flex items-center justify-center text-gray-600 dark:text-gray-300 hover:text-red-500 dark:hover:text-red-500 transition-colors" data-type="products" data-id="${product.id}">
...
const badge = image.badgeId ? getBadgeById(image.badgeId) : null;
const badgeHtml = badge ? `<div class="animated-badge">${badge.name}</div>` : '';
...
// Similarly for image view buttons
